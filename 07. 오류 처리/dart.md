...
class DeviceController {
    ...
    sendShutDown() {
        ...
        if (handle != DeviceHandle.INVALID) {
            ...
            log("Invalid handle for : ${DEV1.toString()}");
        }
    }
    // ...
}
...
class DeviceController {
    ...
    tryToShutDown() {
        final handle = getHandle(DEV1);
        final record = retrieveDeviceRecord(handle);
...
- 단위 테스트에 맞춰 구현한 코드
```dart
List retrieveSection(sectionName) {
...
```dart
// 7-3 예제 : 예외를 던지는 코드
List retriveSection(sectionName) {
    try {
        const stream = FileInputStream(sectionName);
    } catch (e) {
        throw Error("retrieval error", e);
    }
    return [];
}
```
...
List retrieveSection(sectionName) {
    try {
        final stream = FileInputStream(sectionName);
        stream.close();
    } catch (e) {
        throw Error("retrieval error", e);
    }
...
class LocalPort {
    ...
    LocalPort(int portNumber) {
        _innerPort = ACMEPort(portNumber);
    }
...
try {
    final expenses = expenseReportDAO.getMeals(employee.getID());
...
```dart
// 7-6 예제
void registerItm(item) {
    if (item != null) {
        final registry = persistentStore.getItemRegistry();
        if (registry != null) {
            final existing = registry.getItem(item.getID());
...
class MetricsCalculator {
    xProjection(p1, p2) {
        if (p1 == null || p2 == null) {
            throw Error("Invalid argument for MetricsCalculator.xProjection");
        }
        return (p2.x - p1.x) * 1.5;
    }
}
