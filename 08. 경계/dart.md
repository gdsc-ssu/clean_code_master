...
```dart
class Sensors {
    Map _sensers = HashMap();

    Sensor getById(String id) {
        return (Sensor) _sensors.get(id);
    }
}
```
...
```dart
//log4dart 의존성 추가 및 import 선행
void testLogCreate() {
    //1. Logger.root를 통해 log level 세팅
    Logger.root.level = Level.ALL;

    //2. 로거 인스턴스 생성
    final logger = Logger.get("mylogger");
