...
class ReporterConfig {
  ...
  * 리포터 리스너의 클래스 이름
  */
  constructor(m_className: string, m_properties: Property[]) {
...
class TestSuite implements Test {
  constructor(theClass: any, name: string) {
  	this.theClass = theClass;
    this.name = name;
  }
  
  createTest(theClass: any, name: string) {
    // ...
  }
  ...
  TestSuite(theClass: any) {
    // ...
  }
  
  TestSuite(theclass: any, name: string) {
    // ...
  }
}
```
...
class WikiPageResponder implements SecureResponder {
  constructor(page: WikiPage, pageData: PageData, pageTitle: string, request: Request, crawler: PageCrawler) {
   	this.page = page;
  ...
  makeResponse(context: FitnesseContext, request: request) {
  ...
  getPageNameOrDefault(request: Request, defaultPageName: string) {
  ...
  loadPage(resuorce: string, context: FitnesseContext) {
  ...
  notFoundResponse(context: FitnesseContext, request: Request) {
    return new NotFoundResponder().makeResponse(context, request);
  }

  makePageResponse(context: FitnesseContext) {
...
class Assert {
    ...
    asertFalse(message: string, condition: boolean) {
        assertTrue(message, !condition);
    }

    assetFalse(condition: boolean) {
...
```typescript
class FitNesseServer implements SocketServer { constructor(context: FitnesseContext) { this.context = context; } serve(s: Socket) { serve(s, 10000); } serve(s: Socket, requestTimeout: number) { try { const sender = new FitNesseExpediter(s, context); sender.setRequestParsingTimeLimit(requestTimeout); sender.start();} catch(e) {e.printStackTrace(); } } }
```
vs    
```typescript
class FitNesseServer implements SocketServer {
    ...
    serve(s: Socket, requestTimeout: number) { 
        try { 
